...
public class Polynomial {
    ...
    public Polynomial(int[] coeffs) {
        ...
        for(int i = 0; i < coeffs.length; i++) {
            ...
            monomials.push(new Monomial(coeffs[i], i));
        }
    }
    ...
    public void add(int coeff, int degree) {
        ...
        Monomial newM = new Monomial(coeff, degree);

        for(Monomial m : monomials) {
            if(m.combine(newM)) return;
        }
    ...
    public void add(Monomial newM) {
        // TODO: implement this method
        if(newM.eval(1) == 0) return;
    ...
    public String toString() {
        ...
        for(Monomial m : monomials) {
            ...
            for(int i = 0; i < mList.size(); i++) {
                if(!mList.get(i).lowerDegreeThan(m)) {
                    mList.add(i,m);
                    break;
